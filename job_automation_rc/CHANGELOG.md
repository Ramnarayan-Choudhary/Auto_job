...
All notable changes to AutoJob will be documented in this file.
...
### Added
- **Parallel workers for discovery/enrichment** - `autojob run --workers N` enables
...
- **Apply utility modes** - `--gen` (generate prompt for manual debugging), `--mark-applied`,
  `--mark-failed`, `--reset-failed` flags on `autojob apply`
- **Dry-run mode** - `autojob apply --dry-run` fills forms without clicking Submit
...
- **Config YAML not found after install** - moved `config/` into the package at
  `src/autojob/config/` so YAML files (employers, sites, searches) ship with `pip install`
...
- Interactive setup wizard (`autojob init`)
