...
from autojob.config import DB_PATH
from autojob.database import get_connection, init_db, ensure_columns
from autojob.llm import get_client
...
def set_proxy(proxy_str: str | None):
    ...
    if proxy_str:
        from autojob.discovery.jobspy import parse_proxy
...
def _load_base_urls() -> dict[str, str | None]:
    ...
    from autojob.config import load_base_urls
...
def extract_with_llm(page, url: str) -> dict:
    ...
    try:
        ...
        from autojob.discovery.smartextract import extract_json
