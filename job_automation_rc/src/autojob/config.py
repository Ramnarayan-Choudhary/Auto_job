"""AutoJob configuration: paths, platform detection, user data."""
...
APP_DIR = Path(os.environ.get("AUTOJOB_DIR", Path.home() / ".autojob"))

# Core paths
DB_PATH = APP_DIR / "autojob.db"
PROFILE_PATH = APP_DIR / "profile.json"
RESUME_PATH = APP_DIR / "resume.txt"
RESUME_PDF_PATH = APP_DIR / "resume.pdf"
SEARCH_CONFIG_PATH = APP_DIR / "searches.yaml"
ENV_PATH = APP_DIR / ".env"
GMAIL_CREDENTIALS_PATH = APP_DIR / "gmail_credentials.json"
GMAIL_TOKEN_PATH = APP_DIR / "gmail_token.json"
...
def load_profile() -> dict:
    """Load user profile from ~/.autojob/profile.json."""
    import json
    if not PROFILE_PATH.exists():
        raise FileNotFoundError(
            f"Profile not found at {PROFILE_PATH}. Run `autojob init` first."
        )
    return json.loads(PROFILE_PATH.read_text(encoding="utf-8"))


def load_search_config() -> dict:
    """Load search configuration from ~/.autojob/searches.yaml."""
    import yaml
...
DEFAULTS = {
    ...
    "apply_timeout": 300,
    "viewport": "1280x900",
    "email_poll_attempts": 4,
    "email_poll_interval": 10,
}


def load_env():
    """Load environment variables from ~/.autojob/.env if it exists."""
...
TIER_COMMANDS: dict[int, list[str]] = {
    ...
    3: ["apply", "doctor", "import-jobs"],
}


def has_browser_use() -> bool:
    """Return True when the browser-use package is importable."""
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 2 and not any(os.environ.get(k) for k in ("GEMINI_API_KEY", "OPENAI_API_KEY", "LLM_URL")):
        missing.append("LLM API key — run [bold]autojob init[/bold] or set GEMINI_API_KEY")
    if required >= 3:
        if not has_browser_use():
            missing.append("browser-use package — install project dependencies")
        try:
            get_chrome_path()
        except FileNotFoundError:
