...
def _resolve_resume_upload_pdf(job: dict) -> Path:
    ...
    Priority:
      1) AUTOJOB_RESUME_PDF env override (absolute/relative path)
      2) Job tailored resume PDF
      3) ~/.autojob/resume.pdf
    """
    ...
    if override:
        ...
        if candidate.exists():
            return candidate
        logger.warning("AUTOJOB_RESUME_PDF does not exist: %s", candidate)
...
def _load_resume_override_text(upload_pdf: Path) -> str:
    ...
    txt_override = os.environ.get("AUTOJOB_RESUME_TXT", "").strip()
    if txt_override:
...
def build_prompt(job: dict, tailored_resume: str,
    ...
    from autojob.config import load_blocked_sso
...
def build_browser_use_prompt(
    ...
    force_auto_signup = os.environ.get("AUTOJOB_AUTO_SIGNUP_FALLBACK", "1").strip().lower() not in {
        "0",
        "false",
        "no",
        "off",
