...
from autojob.apply import prompt as prompt_mod
from autojob.apply.chrome import (
    BASE_CDP_PORT,
    cleanup_on_exit,
    cleanup_worker,
    kill_all_chrome,
...
from autojob.apply.dashboard import (
    add_event,
    get_totals,
    init_worker,
    render_full,
...
from autojob.apply.verification import verify_apply_result
...
def _load_blocked():
    from autojob.config import load_blocked_sites
...
def _seed_target_url_job(conn, target_url: str, min_score: int) -> None:
    ...
    resume_txt = os.environ.get("AUTOJOB_RESUME_TXT", "").strip()
...
def acquire_job(target_url: str | None = None, min_score: int = 7, worker_id: int = 0) -> dict | None:
    ...
    try:
        ...
        from autojob.config import is_manual_ats
...
def _resolve_resume_pdf_for_upload(job: dict) -> Path | None:
    ...
    override = os.environ.get("AUTOJOB_RESUME_PDF", "").strip()
    if override:
        path = Path(override).expanduser().resolve()
        if path.exists():
            return path
        logger.warning("AUTOJOB_RESUME_PDF not found: %s", path)
...
def _manual_login_timeout_seconds() -> int:
    raw = os.environ.get("AUTOJOB_MANUAL_LOGIN_TIMEOUT", "50").strip()
    try:
        value = int(raw)
    except ValueError:
        value = 50
...
def _make_email_tools(
    ...
    async def wait_for_manual_login(instruction: str = "", timeout_seconds: int = 50):
        ...
        print(
            f"\n[AutoJob][W{worker_id}] Manual login handoff\n"
            f"{details}\n"
...
async def _run_browser_agent(
    ...
    max_steps_env = os.environ.get("AUTOJOB_AGENT_MAX_STEPS", "").strip()
    try:
