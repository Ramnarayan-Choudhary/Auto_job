...
def _resolve_profile_directory(profile_dir: Path) -> str:
    ...
    Priority:
      1) AUTOJOB_CHROME_PROFILE env override (e.g. "Profile 2")
      2) Local State -> profile.last_used
      3) Default
    """
    env_profile = os.environ.get("AUTOJOB_CHROME_PROFILE", "").strip()
...
def launch_chrome(worker_id: int, port: int | None = None,
    ...
    if use_main_profile:
        ...
        refresh_from_main = os.environ.get("AUTOJOB_MAIN_PROFILE_REFRESH", "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
