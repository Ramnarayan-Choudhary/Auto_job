"""AutoJob first-time setup wizard.

Interactive flow that creates ~/.autojob/ with:
...
def _setup_profile() -> dict:
    ...
    console.print(Panel("[bold]Step 2: Profile[/bold]\nTell AutoJob about yourself. This powers scoring, tailoring, and auto-fill."))
    ...
    console.print("\n[bold cyan]Automation Preferences[/bold cyan]")
    confirmation_email = Prompt.ask(
        "Confirmation email recipient",
        default=profile["personal"]["email"],
    )
    profile["automation"] = {
        "email_confirmation_enabled": Confirm.ask("Send confirmation emails after verified submissions?", default=True),
        "confirmation_email": confirmation_email,
        "allow_account_creation": Confirm.ask("Allow employer account creation during apply flows?", default=False),
    }
...
def _setup_ai_features() -> None:
    ...
    if not Confirm.ask("Enable AI scoring and resume tailoring?", default=True):
        console.print("[dim]Discovery-only mode. You can configure AI later with [bold]autojob init[/bold].[/dim]")
        return
    ...
    env_lines = ["# AutoJob configuration", ""]
...
def _setup_auto_apply() -> None:
    """Configure autonomous job application with Gemini/browser-use."""
    console.print(Panel(
        "[bold]Step 5: Auto-Apply (optional)[/bold]\n"
        "AutoJob can autonomously fill and submit job applications\n"
        "using browser-use with Gemini and your local Chrome session."
    ))

    if not Confirm.ask("Enable autonomous job applications?", default=True):
        console.print("[dim]You can apply manually using the tailored resumes AutoJob generates.[/dim]")
        return
    ...
    try:
        ...
        console.print(
            "[yellow]browser-use not found in the active environment.[/yellow]\n"
            "Install project dependencies before using auto-apply."
        )

    console.print(
        f"\n[dim]For Gmail OTP retrieval and confirmation emails, put your Google Desktop OAuth JSON at:[/dim]\n"
        f"[cyan]{GMAIL_CREDENTIALS_PATH}[/cyan]"
    )
    if Confirm.ask("Enable Gmail-based OTP retrieval and confirmation emails?", default=True):
        console.print(
            f"[dim]The OAuth token will be created automatically on first authenticated run at:[/dim]\n"
            f"[cyan]{GMAIL_TOKEN_PATH}[/cyan]"
        )
    else:
        console.print("[dim]Skipped Gmail automation. The browser worker can still apply without mailbox access.[/dim]")
    ...
    if Confirm.ask("Configure CapSolver API key? (optional)", default=False):
        ...
        if ENV_PATH.exists():
            ...
            ENV_PATH.write_text(f"# AutoJob configuration\nCAPSOLVER_API_KEY={capsolver_key}\n", encoding="utf-8")
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]AutoJob Setup Wizard[/bold green]\n\n"
            "This will create your configuration at:\n"
            f"  [cyan]{APP_DIR}[/cyan]\n\n"
            "You can re-run this anytime with [bold]autojob init[/bold].",
            border_style="green",
        )
    )
    ...
    from autojob.config import get_tier, TIER_LABELS, TIER_COMMANDS
    ...
    unlock_hint = ""
    if tier == 1:
        unlock_hint = "\n[dim]To unlock Tier 2: configure an LLM API key (re-run [bold]autojob init[/bold]).[/dim]"
    elif tier == 2:
        unlock_hint = "\n[dim]To unlock Tier 3: install browser-use dependencies + Chrome.[/dim]"
